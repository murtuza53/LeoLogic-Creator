...
import { LoaderCircle, UploadCloud, File, Trash2, Download, TableProperties } from 'lucide-react';
...
export default function JsonToExcelConverter() {
  ...
  const { checkLimit, incrementUsage, isUserLoading } = useUsageLimiter('jsonToExcel');
  ...
  const handleConvert = async () => {
    ...
    try {
        ...
        saveAs(blob, `${file.name.replace(/\.[^/.]+$/, "") || 'data'}.xlsx`);

        toast({
            title: "Conversion Successful!",
            description: `Your JSON file has been converted to Excel and downloaded.`
        });
        setIsLoading(false);
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="mt-8 grid gap-8">
        ...
        <Button onClick={handleConvert} disabled={isLoading || !file} className="w-full bg-accent hover:bg-accent/90 text-accent-foreground font-bold text-base py-6">
            {isLoading ? <><LoaderCircle className="mr-2 h-5 w-5 animate-spin" />Converting...</> : <><TableProperties className="mr-2 h-5 w-5" />Convert to Excel</>}
        </Button>
